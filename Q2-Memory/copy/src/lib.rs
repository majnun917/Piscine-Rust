pub fn nbr_function(c: i32) -> (i32, f64, f64) {
    let new_c: f64 = c as f64;
    return (c, new_c.exp(), new_c.abs().ln())
}

pub fn str_function(a: String) -> (String, String) {
    let mut ex_vec: Vec<f64> = Vec::new();
    for s in a.split_whitespace() {
        let num: f64 = s.parse().unwrap();
        ex_vec.push(num.exp());
    }

    let ex_string = ex_vec.iter()
                          .map(|&x| x.to_string())
                          .collect::<Vec<String>>()
                          .join(" ");
    return (a, ex_string);   
...
pub fn vec_function(b: Vec<i32>) -> (Vec<i32>, Vec<f64>) {
    let mut ex_vec: Vec<f64> = Vec::new();
    for i in &b{
        let new_i: f64 = *i as f64;
        ex_vec.push(new_i.ln());
    }
    return (b, ex_vec);
}
